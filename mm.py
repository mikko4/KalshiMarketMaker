...
class KalshiTradingAPI(AbstractTradingAPI):
    ...
    def place_order(
        ...
        self.logger.info(
            f"Placing {action} order for {side} side at price ${price:.2f} with quantity {quantity}..."
        )
        ...
        try:
            ...
            self.logger.info(
                f"Placed {action} order for {side} side at price ${price:.2f} with quantity {quantity}, order ID: {order_id}"
            )
            return str(order_id)
        except requests.exceptions.RequestException as e:
...
class AvellanedaMarketMaker:
    ...
    def run(self, dt: float):
        ...
        while time.time() - start_time < self.T:
            ...
            self.logger.info(
                f"Current mid price for {self.trade_side}: {mid_price:.4f}, Inventory: {inventory}"
            )

            reservation_price = self.calculate_reservation_price(
                mid_price, inventory, current_time
            )
            bid_price, ask_price = self.calculate_asymmetric_quotes(
                mid_price, inventory, current_time
            )
            ...
            self.logger.info(
                f"Computed desired bid: {bid_price:.4f}, ask: {ask_price:.4f}"
            )
    ...
    def calculate_asymmetric_quotes(
        self, mid_price: float, inventory: int, t: float
    ) -> Tuple[float, float]:
    ...
    def calculate_reservation_price(
        ...
        return (
            mid_price
            + inventory_skew
            - inventory * dynamic_gamma * (self.sigma**2) * (1 - t / self.T)
        )
    ...
    def calculate_optimal_spread(self, t: float, inventory: int) -> float:
        ...
        base_spread = dynamic_gamma * (self.sigma**2) * (1 - t / self.T) + (
            2 / dynamic_gamma
        ) * math.log(1 + (dynamic_gamma / self.k))
    ...
    def manage_orders(
        self, bid_price: float, ask_price: float, buy_size: int, sell_size: int
    ):
    ...
    def handle_order_side(
        self, action: str, orders: List[Dict], desired_price: float, desired_size: int
    ):
        keep_order = None
        for order in orders:
            current_price = (
            ...
            if (
                ...
                keep_order = order
                self.logger.info(
                    f"Keeping existing {action} order. ID: {order['order_id']}, Price: {current_price:.4f}"
                )
            else:
                self.logger.info(
                    f"Cancelling extraneous {action} order. ID: {order['order_id']}, Price: {current_price:.4f}"
                )
                self.api.cancel_order(order["order_id"])

        current_price = self.api.get_price()[self.trade_side]
        if keep_order is None:
            if (action == "buy" and desired_price < current_price) or (
                ...
                try:
                    ...
                    self.logger.info(
                        f"Placed new {action} order. ID: {order_id}, Price: {desired_price:.4f}, Size: {desired_size}"
                    )
                except Exception as e:
                    self.logger.error(f"Failed to place {action} order: {str(e)}")
            else:
                self.logger.info(
                    f"Skipped placing {action} order. Desired price {desired_price:.4f} does not improve on current price {current_price:.4f}"
                )
